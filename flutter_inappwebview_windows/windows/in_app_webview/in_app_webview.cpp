...
namespace flutter_inappwebview_plugin
{
  ...
  void InAppWebView::setPosition(double x, double y, float scale_factor)
  {
    ...
    // client area. (x, y) from Dart's localToGlobal(Offset.zero) is
    // already in client-area logical pixels, so just scale to physical.
    HWND webViewHWnd;
  ...
  InAppWebView::~InAppWebView()
  {
    ...
    if (parentWindow) {
      if (plugin && plugin->windowRegistry) {
        plugin->windowRegistry->DetachForDestroy(parentWindow, "InAppWebView shutdown");
      }
      else {
    ...
    if (parentWindow) {
      if (plugin && plugin->windowRegistry) {
        plugin->windowRegistry->DestroyRegisteredWindow(parentWindow, "InAppWebView shutdown");
      }
      else if (::IsWindow(parentWindow)) {
